    ...
    $.fn.extend({
        ...
        selectdrapdown: function (opt) {
            ...
            $(document).click(function (e) {
                ...
                // }
                var p = $(e.target).parents(".select-drapdown").length;
                if (p == 0) {
                    restore();
                }
            });
            $this.on("click", "li:not(.notdata)", function () {
                //$this.find("li:not(.notdata)").on("click", function () {
                var $th = $(this);
                ...
                //判断input-control的类型，如果是input
                var ic = $th.parents(".select-drapdown").children(".input-control");
                if (ic[0].tagName == "INPUT") {
                    ic.removeClass("placeholder").val($th.text());
                } else {
                    ic.removeClass("placeholder").html($th.html());
                }
