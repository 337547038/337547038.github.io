...
//恢复状态
            ...
            $this.mousemove(function (e) {
                ...
                switch (d) {
                    ...
                    case "bright":
                        ...
                        break;
                    case "tcenter":
                        var objwidth = mt.outerWidth(true);
                        mt.css({
        ...
        inputzoom: function (opt) {
            ...
            $(this).focus(function () {
                var th = $(this);
                var txt = th.val();
                txtchange.focustxt(txt, th.prev(".zoomnum"));
                text = txt; //获取焦点时的值
            }).keydown(function (e) {
                ...
                if (isie && iev == 9.0) {
                    var th = $(this);
                    txtchange.focustxt(th.val(), th.prev(".zoomnum"));
                }
                ...
                function (e) {
                    ...
                    if (v == 1) {
                        ...
                        txtchange.addtxt(zoomnum, append, postion, opt.f);
                    } else if (v == -1) {
                        //删除一个值
                        txtchange.deltxt(zoomnum, postion, opt.f, zoomnum.height(), newtext, keycode);
                    } else if (v > 1) {
                        ...
                        for (var i = 0; i < v; i++) {
                            ...
                            txtchange.addtxt(zoomnum, append, p, opt.f);
                            p++;
                        }
                    } else if (v < -1) {
                        //删除多个时
                        ...
                        for (var i = 0; i < Math.abs(v); i++) {
                            txtchange.deltxt(zoomnum, dp, opt.f, zoomnum.height(), newtext, keycode);
                            dp++;
                        }
                    } else if (newtext != text) { //处理特殊情况，前后两个字符长度一样时，例如选中一个字符然后按下其它键，此时长度没有发生变化，但字符串内容发生了变化
                        txtchange.focustxt(newtext, zoomnum);
                    }
                    text = newtext; //重新赋值
                    return false;
                }).blur(function () {
            ...
            var txtchange = {
                ...
                //格式化ty格式化类型0为手机格式，其他格式每四个一组
                format: function (div, ty) {
                    var s = div.children("span");
                    s.removeClass(opt.c);
                    if (ty == 0) {
                        s.eq(2).addClass(opt.c);
                        s.eq(6).addClass(opt.c);
                        s.eq(10).addClass(opt.c)
                    } else {
                        s.each(function () {
                            var index = s.index(this) + 1;
                            if (index % 4 == 0) {
                                $(this).addClass(opt.c);
                            }
                        })
